...
const HERO_CLIP_START = 9;
const HERO_CLIP_END = 64;
...
function initHeroVideo() {
  ...
  const playFromClipStart = () => {
    heroVideo.currentTime = HERO_CLIP_START;
    heroVideo.play().catch(() => {});
  };

  heroVideo.addEventListener("loadedmetadata", () => {
    playFromClipStart();
    document.querySelector(".hero")?.classList.add("hero--video-ready");
    document.getElementById("hero-aurora")?.classList.add("is-dim");
  });

  heroVideo.addEventListener("timeupdate", () => {
    if (heroVideo.currentTime >= HERO_CLIP_END) {
      heroVideo.currentTime = HERO_CLIP_START;
      heroVideo.play().catch(() => {});
    }
  });

  heroVideo.addEventListener("ended", playFromClipStart);
}
